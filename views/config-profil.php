<div class="container-fluid" id="mon-profil">
    <h2 class="center">Modifier mon profil</h2>
    <div class="col-md-12">
        <h3>Modifier votre mot de passe</h3>
    </div>
    <div class="form-group col-md-12">
        <label class="col-md-2 control-label" for="mdp-label">Mot de passe actuel</label>
        <div class="col-md-4">
            <input id="mdp" name="mdp" type="password" placeholder="Votre mot de passe actuel" class="form-control input-md" required="">
        </div>
    </div>

    <div class="form-group col-md-12">
        <label class="col-md-2 control-label" for="newmdp-label">Nouveau mot de passe</label>
        <div class="col-md-4">
            <input id="newmdp" name="newmdp" type="password" placeholder="Votre nouveau mot de passe" class="form-control input-md" required="">
        </div>
    </div>

    <div class="col-md-12">
        <h3>Changer d'adresse mail</h3>
    </div>

    <div class="form-group col-md-12">
        <label class="col-md-2 control-label" for="email-label">Votre adresse mail actuelle</label>
        <div class="col-md-4">
            <input id="email" name="email" type="email" placeholder="Votre adresse email actuelle" class="form-control input-md" required="">
        </div>
    </div>

    <div class="form-group col-md-12">
        <label class="col-md-2 control-label" for="newemail-label">Votre nouvelle adresse mail</label>
        <div class="col-md-4">
            <input id="email" name="newemail" type="email" placeholder="Votre nouvelle adresse mail" class="form-control input-md" required="">
        </div>
    </div>

    <div class="col-md-12">
        <h3>Supprimer votre compte</h3>
        <div class="col-md-4">
            <input type="checkbox"> Je supprime mon compte définitivement
        </div>
    </div>

    <div class="col-md-12">
        <h3>Confirmer le/les changement(s)</h3>
    </div>

    <div class="form-group col-md-12">
        <label class="col-md-2 control-label" for="mdp-label">Mot de passe actuel</label>
        <div class="col-md-4">
            <input id="mdp" name="mdp" type="password" placeholder="Votre mot de passe actuel" class="form-control input-md" required="">
        </div>
    </div>

</div>